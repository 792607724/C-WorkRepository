...
namespace HIDTool
{
    public partial class HIDToolForm : Form
    {
        ...
        private void activateAgingTestButton_Click(object sender, EventArgs e)
        {
            //MessageBox.Show("老化测试激活操作！");
            string test_time = agingTestTimeTextBox.Text;
            //MessageBox.Show("当前设置老化测试测试时间为：" + test_time);
            /*
            if (float.Parse(test_time) > 240)
            {
                MessageBox.Show("测试时间：" + test_time + "，超出最大值，按最大值已设置：240");
                test_time = "240";
                agingTestTimeTextBox.Text = test_time;
            }*/
            HIDDevice device = OpenHIDDevice(this.comboBoxDevices.SelectedIndex);
            ...
            if (device != null)
            {
                ...
                Array.Copy(Protocol.COMMAND_REQUEST_TYPE_SET_AGEING_ENABLE, requestBuffer, Protocol.COMMAND_REQUEST_TYPE_SET_AGEING_ENABLE.Length);
                // 激活
                requestBuffer[7] = 0x01;
                // 激活测试时间传入
                requestBuffer[9] = (byte)((int)float.Parse(test_time) & 0xff);
                requestBuffer[8] = (byte)(((int)float.Parse(test_time) & 0xff00) >> 8);
                //MessageBox.Show((requestBuffer[8]).ToString() + " + " +(requestBuffer[9]).ToString());
               
                device.Send(requestBuffer, 0, requestBuffer.Length);
                device.Receive(responseBuffer, 0, responseBuffer.Length);
                byte activate_status = responseBuffer[7];
                if (activate_status == 1)
                {
                    MessageBox.Show("老化测试激活成功！");
                }
                else
                {
                    MessageBox.Show("老化测试激活失败！");
                }
        ...
        private void clearAgingTestButton_Click(object sender, EventArgs e)
        {
            //MessageBox.Show("老化测试清除操作！");
            HIDDevice device = OpenHIDDevice(this.comboBoxDevices.SelectedIndex);
            ...
            if (device != null)
            {
                ...
                if (activate_status == 1)
                {
                    MessageBox.Show("老化测试清除成功！");
                }
                else
                {
                    MessageBox.Show("老化测试清除失败！");
                }
        ...
        private void agingTestTimeTextBox_TextChanged(object sender, EventArgs e)
        {
