...
namespace HIDInterface
{
    ...
    public class HIDDevice
    {
        ...
        public static HIDDevice Open(HIDDeviceInfo deviceInfo, bool isAsync = false)
        {
            ...
            if (isAsync)
            {
                device.m_CancellationTokenSource = new CancellationTokenSource();
            }
        ...
        public async void SendAsync(byte[] data, int offset, int count)
        {
            if (data.Length > m_Capabilities.OutputReportByteLength)
            {
                throw new Exception("Output report must not exceed " + (m_Capabilities.OutputReportByteLength - 1).ToString() + " bytes");
            ...
            if (m_Stream.CanWrite && m_CancellationTokenSource != null)
            {
                await m_Stream.WriteAsync(data, 0, data.Length, m_CancellationTokenSource.Token);
            }
            else
            {
                throw new Exception("Stream can not write");
        ...
        public async Task ReceiveAsync(byte[] data, int offset, int size)
        {
            if (data.Length > m_Capabilities.InputReportByteLength)
            {
                throw new Exception("Input report must not exceed " + (m_Capabilities.InputReportByteLength - 1).ToString() + " bytes");
            ...
            if (m_Stream.CanRead && m_CancellationTokenSource != null)
            {
                await m_Stream.ReadAsync(data, 0, data.Length, m_CancellationTokenSource.Token);
            }
            else
            {
                throw new Exception("Stream can not read");
