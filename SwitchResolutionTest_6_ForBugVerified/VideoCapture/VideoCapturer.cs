...
namespace VideoCapture
{
    public class VideoCapturer : IDisposable
    {
        ...
        public bool StartupCapture(CameraInfo cameraInfo, int cameraId)
        {
            ...
            DsError.ThrowExceptionForHR(hr);
            // --Guangtao Modified
            if (m_PreviewWidth == 3840)
            {
                Console.WriteLine(m_PreviewWidth);
                SetConfigParamsH264(m_CaptureGraphBuilder, sourceFilter, 30, m_PreviewWidth, m_PreviewHeight);
            }
            else
            {
        ...
        // --Guangtao Modified
        private void SetConfigParamsH264(ICaptureGraphBuilder2 capGraph, IBaseFilter capFilter, int iFrameRate, int iWidth, int iHeight)
        {
            int hr;
            object config;
            AMMediaType media_type = null;
            ...
            for (int i = 0; i < piCount; i++)
            {
                ...
                // MJPG -- Guangtao
                if (videoInfoHeader.BmiHeader.Width == iWidth && videoInfoHeader.BmiHeader.Height == iHeight &&
                    media_type.subType == MediaSubType.H264)
                {
                    hr = videoStreamConfig.SetFormat(media_type);
                    DsError.ThrowExceptionForHR(hr);
                    break;
